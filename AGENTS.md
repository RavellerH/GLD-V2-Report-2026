...
- 🆕 **10 Sep — status proyek dikonfirmasi:** RU IV Cilacap **baru survey lokasi** (belum instalasi/commissioning/validasi lapangan); sertifikasi masih **persiapan dokumen**; thermal camera **dihapus dari scope aktif** (proposalnya tetap arsip); dan pekerjaan lanjutan chamber Week 2 September dikonfirmasi **selesai** tanpa bukti tertulis pasca-laporan. Angka progres **44% vs baseline 43% (4 Sep)** tidak dinaikkan karena belum ada metrik baru yang terukur.
- 🆕 **10 Sep — audit Termin 1 sertifikasi:** paket dua-termin **belum siap diajukan**. Bukti keberadaan prototipe ada, tetapi bukti uji enclosure, iterasi traceable, witness/validasi Pertamina, berita acara, dan laporan pekerjaan khusus Termin 1 belum ditemukan. Matriks gap + template kerja tersedia di `Paket Pertamina/02_Sertifikasi_ATEX_IECEx/` (dec:80).
- 🆕 **11 Sep — paket Termin 1 field testing 20% siap-zip:** folder ringkas `Paket Pertamina/03_Pengajuan_Termin_1_FieldTesting_20Persen_Rev02/` memuat 26 file (sekitar 28,79 MB), termasuk laporan Rev02, notulen witness 6 Agustus yang sudah dikoreksi, dan draft BAST generik DOCX/PDF. BAST masih perlu diisi data SPK, nominal, identitas penandatangan, dan tanda tangan sebelum pengajuan resmi (dec:97).
- 🆕 **11 Sep — laporan Termin 1 field testing 20% disiapkan:** berdasarkan konfirmasi PIC bahwa kunjungan PT Pertamina Patra Niaga ke **Lab IoT, Instrumentation and Computations, Gedung Laboratorium Fisika Terpadu, Institut Teknologi Bandung**, sebelum visit Cilacap merupakan witness perkembangan/integrasi untuk tahap FAT/FIT. Pihak yang terlibat: **LAPI Ganesha Utama, Lab IoT/Instrumentation and Computation ITB, dan PT Pertamina Patra Niaga**. Paket bukti engineering, kesiapan perangkat, konfigurasi firmware, integrasi end-to-end, mesh, dan alarm push dirangkum dalam `Paket Pertamina/01_Pilot_Field_Testing/Laporan_Pemenuhan_Deliverable_Termin_1_FieldTesting_GLD.{docx,pdf}`. Kesimpulan: **layak diajukan untuk evaluasi, penerimaan, dan pembayaran 20%**, dengan tanda tangan pengesahan laporan/BAST sebagai penutupan administratif. Ini tidak mengubah status instalasi/SAT RU IV maupun status Termin 1 sertifikasi (dec:95).
...
- ⚠️ **Blocker/gate:** autonomi baterai GLD (angka pack final berubah dan belum diberikan); TPL5010/DC converter; TRA/JSA; scope ATEX; **sampel gas tambahan**; adapter MQTT→backend; baseline DB; serta rekonsiliasi TDS AI/Ethernet. Mitigasi sampel: portable chamber selesai untuk dibawa ke **kantor HQ kilang** (bukan area proses) saat sampel tersedia, untuk pengambilan data dan training/validasi model di lokasi; pencarian suplier/penyediaan Pertamina tetap paralel. Jangkauan LoRa per-hop **disiasati mesh** (bukan blocker keras).
...
- 🆕 **10 Sep — Checklist Kesiapan Instalasi RU IV Cilacap dibuat** (`Deliverables/Checklist_Kesiapan_Instalasi_RU-IV_Cilacap.html`): jalur kantor HQ/uji terkendali/permanen dipisahkan, PIC dan go/no-go pra-mobilisasi diringkas, serta acceptance test disiapkan. Datasheet Sistem lengkap sudah tersedia sebagai acuan; instalasi permanen area berbahaya tetap belum GO sampai izin/HSE dan gate sertifikasi relevan ditutup (dec:78).
