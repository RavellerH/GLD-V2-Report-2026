...
- 🔵 **Model AI baru: CNN Dual-Branch (9 Agu, ditemukan di `Sumber Dokumen/`, push langsung Farhan di luar sesi Claude):** penerus **CNN 1D**, klasifikasi 3-kelas sama (LPG/CO2/Udara Bersih), arsitektur 2-cabang (Conv1D sensor + Dense datasheet-sensitivity). Akurasi naik ke **99,73% (PC) / 99,20% (on-chip ESP32-S3 int8, 9,14 KB)** vs 97,6%/97,3% CNN 1D. ⚠️ **Masih TIDAK mencakup CO/H2S/Benzena** (requirement 6 Agu) — `gate:gas-extra` belum tertutup. ⚠️ 2 hal perlu verifikasi ke tim: slide uji-lapangan sebut kelas "H2" yang tak konsisten dgn skema 3-kelas, dan headline 99,73% keliru mengutip angka pre-kuantisasi bukan angka on-chip. **Belum ada arahan apakah model ini menggantikan CNN 1D di deliverable.** Detail → `memory/entities.md` § Model AI, `memory/decisions.md` dec:36.
