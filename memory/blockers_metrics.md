...
## Status progres (di-reassess 17 Sep 2026 — sebelumnya per 4 Sep 2026, per 24 Jul 2026)
- **Progres pilot Cilacap: 49%** vs baseline resmi **52%** (per 17 Sep) → **-3 poin** (murni sisi engineering lab, BUKAN kesiapan lapangan; jangan over-claim). Melanjutkan tren menyusut: **+20 poin (24 Jul) → +1 poin (4 Sep) → -3 poin (17 Sep)** — bukan krn progres melambat, tapi krn sisa pekerjaan makin bergantung gate eksternal (scoping ATEX, TRA/JSA, sampel gas) yg tak bisa dipercepat murni lewat rekayasa & rencana terus naik.
- **Metodologi reassessment** (rata-rata 12 aktivitas Gantt baseline Kick-Off, tidak berubah): perubahan 17 Sep — "Site survey" 65%→75% (checklist &amp; bracket diserahkan vendor, lokasi Jalur C dikonfirmasi perimeter SRU), "Detailed design" 90%→92% (casing v3 cadangan sertifikasi), "Prototype build" 62%→68% (laporan chamber gas versi terbaru 14 Sep), "Lab test" 55%→65% (uji ketahanan RF thd interferensi 17 Sep), "Integration test" 75%→80% (6 gate ditutup), "HSE review & permit prep" 5%→22% (checklist instalasi, daftar persiapan vendor, draf permintaan material — TRA/JSA tetap belum disahkan), "AI model training/validation" 75%→**85%** (dinaikkan eksplisit atas permintaan user, alasan: model sudah mendekati final utk gas yg sudah dicakup). Aktivitas lapangan murni (field install/trial/evaluation) **tidak berubah** — masih 0%, krn belum ada eksekusi fisik RU.
...
- Detail reassessment 4 Sep → `decisions.md` dec:62. Detail reassessment 17 Sep (angka terkini di atas) → `decisions.md` dec:121.
...
- ~~gate:mqtt-adapter~~ → **DIKONFIRMASI SELESAI 4 Sep** (user langsung): adapter MQTT→backend sudah dibangun, menutup gap yang sempat ditemukan audit repo terkunci (commit `660febbf`/`69a493c3`, dec:56). Jalur data lapangan→dashboard kini tersambung penuh di level aplikasi.
- ~~gate:net-registration~~ / ~~"satukan jaringan Gateway+server"~~ → **TIDAK RELEVAN** (dikonfirmasi user 4 Sep): field network LGU independen (GW→CH→PC-server via router LGU sendiri, dec:46) — tidak perlu registrasi MAC Gateway atau penyatuan jaringan ke IT kilang. Yang tetap perlu koordinasi dgn Pertamina hanya sisi dashboard-access (NIC kedua PC dedicated ke intranet kantor) — lih. `gate:dashboard-exposure` yang masih terbuka.
- ~~gate:ch-power-spec~~ → **DIREKONSILIASI 4 Sep** (dikonfirmasi user): input power CH 5VDC (spek EMC) adalah titik referensi internal board (post-regulator), bukan kontradiksi dgn konfigurasi solar+18650 — dua hal berbeda, bukan gap.
- ~~gate:db-baseline~~ → **BUKAN BLOCKER** (dikonfirmasi user 4 Sep): SQLite adalah keputusan sadar dalam arsitektur sistem independen LGU (PC server + jaringan sendiri), bukan menunggu keputusan Pertamina. Migrasi ke Postgres/TimescaleDB tetap opsi masa depan bila volume rollout 6-RU membutuhkannya, tapi tidak menahan commissioning pilot RU IV.
- ~~"data collision belum diuji"~~ (dari 30 Jul) → **SUDAH DIUJI** (dikonfirmasi user 4 Sep, detail hasil belum didokumentasikan tertulis).
- ~~blk:power~~ / ~~blk:tpl~~ / ~~blk:conv~~ (autonomi baterai GLD, TPL5010, DC converter) → **DIDEPRIORITASKAN** (dikonfirmasi user 4 Sep): GLD versi baterai portable **tidak digunakan untuk saat ini** — fokus deployment adalah 24VDC saja. Ketiga item ini terkait jalur baterai portable, jadi tidak lagi relevan sbg blocker aktif. Bukan diselesaikan secara teknis — jalur baterai portable bisa dilanjutkan terpisah di masa depan bila diperlukan.
- ~~AI on-device belum diverifikasi dokumen tertulis~~ → **DIPERKUAT** (dikonfirmasi user 4 Sep): status fungsional **didemokan langsung sebelum kunjungan Cilacap** (sebelum 9-10 Agu). Dokumen firmware tertulis terpisah masih menyusul — bukan lagi murni klaim verbal, tapi didukung demo.
- ~~gate:cert-height~~ (untuk LGU) → **BUKAN TANGGUNG JAWAB LGU** (dikonfirmasi user 4 Sep, "ini penting, ingat"): **instalasi fisik RU IV Cilacap dilakukan vendor Pertamina**, termasuk sertifikasi HSE ketinggian — LGU berperan **supervisi teknis & QA** saja, bukan eksekutor fisik. Ini mengubah asumsi dasar JSA-02/JSA-03 (draft JSA/HSE direvisi total, lih. dec:63) — sebelumnya diasumsikan tim LGU yang fisik memasang.
- **gate:dashboard-exposure diperjelas** (dikonfirmasi user 4 Sep, TETAP TERBUKA sbg gate formal): PC dedicated LGU hanya expose **API/dashboard endpoint** ke intranet kantor kilang via NIC kedua, bukan akses jaringan penuh — mengurangi risiko yg sempat dikhawatirkan, tapi belum ada review keamanan formal.
- **gate:capacity — mitigasi jelas, gate tetap terbuka** (dikonfirmasi user 4 Sep): bisa diatur via **adjust interval kirim per sensor**, trade-off dgn bandwidth LoRa yg terbatas. Angka resmi/load test formal tetap belum ada.
...
| ID | Gate | Status |
|---|---|---|
| gate:jsa | TRA/JSA | Survey RU IV telah selesai; status pengesahan TRA/JSA tidak dikonfirmasi. Untuk **instalasi** tetap diperlukan izin kerja/HSE dan dokumen yang disahkan |
| gate:atex-docs | Dokumentasi sertifikasi ATEX | **Persiapan dokumen** per konfirmasi user 10 Sep; belum boleh dianggap sudah diajukan, diuji, atau tersertifikasi |
| gate:cert-height | Sertifikasi ketinggian pemasangan | ⚠️ **Update 4 Sep**: percobaan di kilang baru setinggi orang, belum tersertifikasi utk instalasi permanen (rapat resmi 6 Agu) — **TAPI ini kini tanggung jawab vendor Pertamina yg eksekusi fisik instalasi, bukan LGU** (dikonfirmasi user). Gate tetap terbuka secara umum (perlu Pertamina/vendor selesaikan), tapi bukan lagi blocker bagi LGU |
| gate:gas-extra | Requirement gas tambahan (Benzena/CO/H2S) | Belum direkonsiliasi dgn gate 6-kelas (dec:17/dec:30) — status verifikasi belum ada. Model AI resmi saat ini (**CNN Dual-Branch**, dec:36) sudah cakup H2 (gate lama) tapi **masih TIDAK cakup H2S/Benzena** — gate ini belum tertutup. ⚠️ **Blocker awal (dec:48, ~awal Sep)**: sampel gas (H2S, metana, amonia, dll.) dari Pertamina **belum diterima sampai sekarang**; LGU sudah cari suplier di Bandung (masih aktif, **lebih cepat bila Pertamina sediakan langsung**) tapi **masih sulit dapat sampel gas-gas ini** — tanpa sampel, dataset/model utk gas-gas tambahan ini tidak bisa dikembangkan/divalidasi. ✅ **Update 18 Sep (dec:123)**: sampel **H2S ternyata sudah disiapkan RU IV Cilacap** — blocker bergeser dari ketersediaan sampel ke **logistik pengiriman** (lih. `gate:hazmat-gas-logistics`); rencana kerja: chamber dibawa ke Cilacap, training on-site pakai sampel yg sudah ada di RU (bukan menunggu sampel dikirim ke Bandung). **Mitigasi (dec:77)**: portable chamber yang sudah selesai dibawa ke **kantor HQ kilang** (bukan area proses) untuk pengambilan data serta training/validasi model di lokasi saat sampel tersedia. Ini alternatif; pencarian suplier/penyediaan Pertamina tetap paralel. ✅ **Digeneralisasi jadi metodologi rollout (dec:108, 17 Sep)**: chamber dibawa ke tiap lokasi kilang (bukan cuma Cilacap), dataset+training+inject firmware dilakukan on-site, gas yang dilatih **disesuaikan permintaan tiap RU** — implikasinya model AI GLD berpotensi **tidak seragam antar-RU** (belum jelas apakah 4-kelas CNN Dual-Branch jadi baseline wajib + tambahan per lokasi, atau beda total per RU — perlu diklarifikasi ke depan). |
| gate:tds-ai-profile | Profil AI TDS R4 vs model resmi | 🆕 TDS GasleakDetector R4 (4 Sep) mengonfirmasi inferensi lokal secara generik, tetapi hanya menyebut **Clean Air/LPG/H2** dan tidak menyebut CO2, nama CNN Dual-Branch, akurasi 99,20%, kuantisasi int8, atau ukuran 9,14KB. Ini **belum membatalkan dec:36** (konfirmasi user: CNN Dual-Branch 4 kelas LPG/CO2/Clean Air/H2); perlu revisi TDS atau bukti firmware/model tertulis agar klaim client-facing sepenuhnya sinkron. |
| gate:capacity | Kapasitas GW/CH vs jumlah sensor | TDS R4 kini memberi **resource limits firmware**, bukan sizing deployment: CH 8 kandidat parent, 32 cache node, 8 alarm queue, 8 TX queue, 16 pending downlink; Gateway 8 pesan MQTT volatil/≤1023B. **Maksimum sensor/CH, CH/GW, throughput, collision margin, dan capacity acceptance test end-to-end tetap belum ada** — action item dec:28 masih terbuka. ⚠️ **Mitigasi (dec:63, 4 Sep)**: bisa diatur via adjust interval kirim per sensor (trade-off bandwidth LoRa) — jalur mitigasi jelas, tapi angka resmi tetap belum ada. |
| gate:telemetry-cadence | Interpretasi interval endpoint vs polling Server | 🆕 TDS GLD R4 menyebut scan 500ms, laporan radio periodik 10s, receive window 2s; TDS Whole System menyebut telemetri normal cache/pull. Dec:43 menyebut alarm push ~10s dan normal pull/polling ~90s. Interpretasi paling konsisten: **10s GLD→CH**, **~90s Server→CH**, tetapi pembagian layer ini belum dinyatakan eksplisit pada satu sumber—wajib dibedakan dalam diagram/deliverable sampai dikonfirmasi. |
...
| gate:atex-scoping | Kelompok gas (IIC/IIB/IIA), kelas temperatur (T1–T6), klasifikasi zona (0/1/2 atau 20/21/22) utk GLD V2 | ⚠️ **DIREVISI 5 Sep (dec:68)** — **sebagian sudah settled**, bukan "sama sekali belum ditentukan" spt tercatat 27 Agu (temuan itu belum sempat menyilangkan proposal formal). Proposal sertifikasi (`PROPOSAL SERTIFIKASI...[21 FEB 2026].pdf`) sudah eksplisit menargetkan **Zona 1, Kategori 2G, Grup II, T4 (≤135°C)**. Yang masih genuinely terbuka: **grup gas spesifik (IIC/IIB/IIA)** — direkomendasikan **IIC** (dec:68, krn H2 termasuk 4 kelas gas resmi model AI) — dan **metode proteksi Ex i vs Ex d** (belum diputuskan, proposal sengaja sisakan sbg keputusan kick-off). Sensor MQ heater ~300–400°C tetap risiko utama T4 — belum ada perhitungan/pengukuran hot-spot sama sekali (blocker teknis, bukan cuma dokumentasi). Material enclosure: **Aluminum alloy + Stainless steel** (grade spesifik blm ditentukan). Detail lengkap → `Deliverables/Dashboard_Sertifikasi_GLD_ATEX_IECEx.html` |
| gate:atex-basic-info | Informasi dasar aplikasi & organisasi (formulir ExCB, izin usaha, struktur org, profil fasilitas manufaktur, ISO 9001) | 🆕 **Gate baru (dec:68, 5 Sep)** — ditemukan dari dokumen asli Inggris `IECEx ATEX Certification Information Requirements_EN.pdf` (diberikan user langsung dari ExCB) yg punya kategori "Basic Information" terpisah dari dokumentasi teknis — kategori ini **belum pernah dilacak sama sekali** di checklist/gap-analysis repo `repo:cert` sebelumnya (yg hanya cover bagian teknis+sampel). Bukan gate teknis, tapi tetap wajib sebelum submission bisa jalan |
| gate:atex-flame-arrestor | Flame arrestor sinter-metal bersertifikat di elemen sensing sensor MQ | 🆕 **Gate baru (dec:69, 5 Sep)** — ditemukan saat verifikasi klaim suhu heater MQ: elemen sensing MQ sengaja terekspos ke udara sekitar (lewat mesh, bukan tersembunyi di enclosure tersegel) agar bisa mendeteksi gas — produk komersial sejenis biasanya pakai flame arrestor sinter-metal bersertifikat Ex di atas elemen sensing utk mencegah penyalaan merambat keluar. Item BOM ini **belum pernah disebut sama sekali** di checklist §1.3 (BOM) manapun — kemungkinan celah tambahan yg belum teridentifikasi tim sertifikasi |
| gate:server-spec | Spesifikasi PC fisik dekat Gateway (dec:46), disediakan Pertamina | LGU sudah usulkan rekomendasi minimum (dec:44) — **menunggu konfirmasi/pengadaan Pertamina**. Field network sendiri sudah independen (lih. resolved di atas) — gate ini murni soal pengadaan unit fisiknya |
| gate:gw-ethernet | Gateway punya port Ethernet fisik (spek EMC) selain Wi-Fi | **Konflik sumber makin jelas (dec:57):** spek EMC dec:45 menyebut port Ethernet tersedia, tetapi TDS Gateway R4 menyatakan interface native hanya **Wi-Fi STA**, “Ethernet native: tidak”, dan Wi-Fi→Ethernet hanya aksesori eksternal. Perlu klarifikasi apakah port fisik benar ada tetapi belum didukung firmware, atau spek EMC/TDS salah. |
| gate:dashboard-exposure | Keamanan akses dashboard dari PC server LGU ke LAN kantor kilang | **Direncanakan langsung via NIC kedua** (dec:46) — belum ada review keamanan formal. ⚠️ **Diperjelas (dec:63, 4 Sep)**: akses dibatasi hanya ke API/dashboard endpoint, bukan akses jaringan penuh — mengurangi risiko eksposur ke seluruh intranet kantor. Satu-satunya sisi jaringan yang masih perlu koordinasi dgn Pertamina, karena field network sudah independen |
| gate:hazmat-gas-logistics | Logistik pengiriman sampel gas berbahaya (H2S dkk.) | 🆕 **Gate baru (dec:106, 17 Sep)** — pengadaan sampel gas berbahaya (`gate:gas-extra`) tersendat bukan cuma krn ketersediaan supplier, tapi krn **gas (termasuk H2S) tergolong Dangerous Goods** di bawah UU Pos No.38/2009 dan **tidak bisa dikirim via kurir umum** (JNE/J&T menolak barang mudah terbakar/gas) — diverifikasi via web search. H2S lebih ketat lagi (toksik + mudah terbakar), butuh kurir kargo hazmat berlisensi dgn kemasan/izin khusus. ✅ **Mitigasi eksplisit (dec:108)**: bukan gas yang dikirim ke lab, tapi **chamber portable yang dibawa ke lokasi kilang** — dataset diambil & model dilatih on-site, lalu di-inject ke firmware di tempat. Ini jadi pola standar rollout 6 RU (bukan hanya Cilacap), sekaligus alasan efisiensi waktu (tak perlu menunggu logistik sampel). ✅ **Kasus konkret pertama (dec:123, 18 Sep)**: sampel H2S **sudah disiapkan RU IV Cilacap** — kendala murni pengiriman (bukan lagi ketersediaan, lih. `gate:gas-extra`), rencana: chamber dibawa ke Cilacap, training dilakukan on-site pakai sampel yg sudah ada di RU |
| gate:area-classification-writeup | Konfirmasi tertulis klasifikasi area (safe/hazardous) per titik pasang RU IV Cilacap | 🆕 Muncul dari klarifikasi arah instalasi (dec:106, 17 Sep): rencana **Jalur C (instalasi permanen) akan dijalankan hanya di lokasi yang tidak memerlukan ATEX** (safe area) — tapi klasifikasi resmi area tsb dari HSE/Engineering RU IV **belum ada bukti tertulis**. Tanpa ini, klaim "lokasi X tidak perlu ATEX" masih asumsi kerja, bukan keputusan resmi Pertamina. ✅ **Menyempit (dec:109, 17 Sep)**: lokasi sudah dikonfirmasi Pertamina — **area aman di perimeter Sulfur Recovery Unit (SRU)**, bukan di dalam proses SRU. Gate tetap terbuka, tapi kini murni menunggu **dokumen klasifikasi area tertulis resmi** dari HSE/Engineering RU IV utk lokasi yg sudah pasti, bukan lagi menunggu keputusan lokasi itu sendiri |
