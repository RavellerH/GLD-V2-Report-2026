...
| gate:capacity | Kapasitas GW/CH vs jumlah sensor | TDS R4 kini memberi **resource limits firmware**, bukan sizing deployment: CH 8 kandidat parent, 32 cache node, 8 alarm queue, 8 TX queue, 16 pending downlink; Gateway 8 pesan MQTT volatil/≤1023B. **Maksimum sensor/CH, CH/GW, throughput, collision margin, dan capacity acceptance test end-to-end tetap belum ada** — action item dec:28 masih terbuka. |
...
| gate:db-baseline | Baseline database produksi RU (SQLite pilot vs PostgreSQL/TimescaleDB vs MySQL dataset) | RU saat ini pakai **SQLite file-based** (dec:56); migrasi ke Postgres/TimescaleDB belum jadi keputusan resmi. TDS Server R4 menambah **MySQL/CSV hanya untuk dataset engineering**, eksplisit bukan storage seluruh telemetri/alarm/topologi. Ketiga lapisan harus dipisahkan dan dipetakan sebelum sizing, backup, retention, HA, dan recovery objective ditetapkan. |
