...
| `06 Aguatus Deteksi_Kebocoran_Gas_CNN_DualBranch (2)  -  Repaired.pptx` | Presentasi **CNN Dual-Branch** — **penerus CNN 1D** (Lab IoT ITB; dibuat 29 Jul, edit terakhir 6 Agu oleh **Ilmania Syakira**; **di-push langsung ke `main` oleh Farhan Budiman 9 Agu di luar sesi Claude ini**, commit "AI update"). Klasifikasi 3 kelas sama (LPG/CO2/Udara Bersih) tapi arsitektur baru 2-cabang (Conv1D dari 8 sensor + Dense dari 7 fitur sensitivitas datasheet MQ) → Concatenate → Dense→Softmax, 1.347 parameter. Akurasi **99,73% PC / 99,20% on-chip int8** (9,14 KB), naik signifikan dari CNN 1D. ⚠️ **Belum menutup requirement CO/H2S/Benzena** (dec:30) — masih 3-kelas lama. ⚠️ 2 inkonsistensi di deck (slide "uji lapangan" sebut kelas "H2" yg tak konsisten dgn skema 3-kelas; headline 99,73% keliru mengutip angka pre-kuantisasi) — **belum diverifikasi ke tim sumber**. Detail lengkap → `entities.md` § Model AI, `decisions.md` dec:36. |
